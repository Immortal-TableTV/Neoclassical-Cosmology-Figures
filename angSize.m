...
% The vertical axis is F_theta, the horizontal axis is redshift.
%
% F_theta = (1 + z) / (H0*a0*r/c)
%
...
% Redshift Values
zvals = linspace(0.0001, 10, 300);     % Avoid z = 0 because F_theta diverges
...
for i = 1:length(Omega_m_vals)
    ...
    for j = 1:length(zvals)
        ...
        % Angular size factor:
        % F_theta = (1 + z) / (H0*a0*r/c)
        Ftheta_vals(j) = (1 + z) / H0a0r_vals(j);

    end

    max_Ftheta = max(max_Ftheta, max(Ftheta_vals));

    % Plot one curve for this Omega_m
    plot(zvals, Ftheta_vals, ...
        'LineStyle', '-', ...
        'LineWidth', 2, ...
        'Marker', 'none', ...
        'Color', colors(i,:), ...
...
ylabel('Angular Size Factor F_\theta')
ylim([0 10])

xlabel('Redshift z')

if choice == 1
    title('Angular Size as a Function of Redshift: Flat Models')
elseif choice == 2
    title('Angular Size as a Function of Redshift: No Dark Energy Models')
end
